...
class IshiTopBanner extends ObjectModel
{
	public $url;
	public $title;
	public $subtitle;
	...
	public static $definition = array(
		'table' => 'ishitopbanners_slides',
		'primary' => 'id_ishitopbanners_slides',
		'multilang' => true,
		'fields' => array(
			...
			'textbgcolor' =>	array('type' => self::TYPE_STRING, 'required' => true, 'size' => 255),
	...
	public function add($autodate = true, $null_values = false)
	{
		...
		$res &= Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'ishitopbanners` (`id_shop`, `id_ishitopbanners_slides`)
	...
	public function delete()
	{
		...
		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'ishitopbanners`
			WHERE `id_ishitopbanners_slides` = '.(int)$this->id
		);
	...
	public function reOrderPositions()
	{
		...
		$max = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			WHERE hss.`id_ishitopbanners_slides` = hs.`id_ishitopbanners_slides` AND hs.`id_shop` = '.(int)$id_shop
		);
		...
		$rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hss.`position` as position, hss.`id_ishitopbanners_slides` as id_slide
			FROM `'._DB_PREFIX_.'ishitopbanners_slides` hss
			LEFT JOIN `'._DB_PREFIX_.'ishitopbanners` hs ON (hss.`id_ishitopbanners_slides` = hs.`id_ishitopbanners_slides`)
