...
include_once(_PS_MODULE_DIR_.'ishitopbanners/IshiTopBanner.php');

class IshiTopBanners extends Module implements WidgetInterface
{
    protected $_html = '';
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->l('Ishi Top Banners');
        $this->description = $this->l('Adds responsive banners with text to your site.');
        $this->ps_versions_compliancy = array('min' => '1.7.0.0', 'max' => _PS_VERSION_);

        $this->templateFile = 'module:ishitopbanners/views/templates/hook/ishitopbanners.tpl';
    }
    ...
    protected function installSamples()
    {
        ...
        for ($i = 1; $i <= 2; ++$i) {
            ...
            foreach ($languages as $language) {
                ...
                $slide->image[$language['id_lang']] = 'topbanner-'.$i.'.png';
            }
            $slide->add();
        }
    }
    ...
    protected function createTables()
    {
        ...
        $res = (bool)Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'ishitopbanners` (
                `id_ishitopbanners_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
                `id_shop` int(10) unsigned NOT NULL,
                PRIMARY KEY (`id_ishitopbanners_slides`, `id_shop`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
        ');

        /* Slides configuration */
        $res &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'ishitopbanners_slides` (
              `id_ishitopbanners_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
              ...
              `textbgcolor` varchar(255) NOT NULL,
              `textalignment` varchar(255) NOT NULL,
              PRIMARY KEY (`id_ishitopbanners_slides`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
        ');

        /* Slides lang configuration */
        $res &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'ishitopbanners_slides_lang` (
              `id_ishitopbanners_slides` int(10) unsigned NOT NULL,
              ...
              PRIMARY KEY (`id_ishitopbanners_slides`,`id_lang`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
        ');
    ...
    protected function deleteTables()
    {
        ...
        foreach ($slides as $slide) {
            $to_del = new IshiTopBanner($slide['id_slide']);
            $to_del->delete();
        }

        return Db::getInstance()->execute('
            DROP TABLE IF EXISTS `'._DB_PREFIX_.'ishitopbanners`, `'._DB_PREFIX_.'ishitopbanners_slides`, `'._DB_PREFIX_.'ishitopbanners_slides_lang`;
        ');
    }
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('submitSlide') || Tools::isSubmit('delete_id_slide') ||
            ...
            if ($mode == 'add') {
                ...
                $associated_shop_ids = IshiTopBanner::getAssociatedIdsShop((int)Tools::getValue('id_slide'));
    ...
    protected function _postProcess()
    {
        ...
        if (Tools::isSubmit('changeStatus') && Tools::isSubmit('id_slide')) {
            $slide = new IshiTopBanner((int)Tools::getValue('id_slide'));
            if ($slide->active == 0) {
                $slide->active = 1;
            } else {
                $slide->active = 0;
            ...
            /* Sets description color */
            $slide->descriptioncolor = Tools::getValue('descriptioncolor');
            /* Sets text background color */
            $slide->textbgcolor = Tools::getValue('textbgcolor');
            ...
            $slide = new IshiTopBanner((int)Tools::getValue('delete_id_slide'));
            $res = $slide->delete();
            $this->clearCache();
            if (!$res) {
    ...
    public function getWidgetVariables($hookName = null, array $configuration = array())
    {
        ...
        if (is_array($slides)) {
            foreach ($slides as &$slide) {
                ...
                list($r, $g, $b) = sscanf($slide['descriptioncolor'], "#%02x%02x%02x");
                $rgb_color = 'rgba('. $r .', ' . $g .', ' .$b .', 0.3'. ')';
                $slide['descriptionbordercolor'] = $rgb_color;
            }
        }

        return array(
            'ishitopbanners' => array(
                'slides' => $slides,
            ),
        );
    }
    ...
    public function hookActionShopDataDuplication($params)
    {
        Db::getInstance()->execute('
            INSERT IGNORE INTO '._DB_PREFIX_.'ishitopbanners (id_ishitopbanners_slides, id_shop)
            SELECT id_ishitopbanners_slides, '.(int)$params['new_id_shop'].'
            FROM '._DB_PREFIX_.'ishitopbanners
    ...
    public function getNextPosition()
    {
        $row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('
            ...
            FROM `'._DB_PREFIX_.'ishitopbanners_slides` hss, `'._DB_PREFIX_.'ishitopbanners` hs
            WHERE hss.`id_ishitopbanners_slides` = hs.`id_ishitopbanners_slides` AND hs.`id_shop` = '.(int)$this->context->shop->id
        );
    ...
    public function getSlides($active = null)
    {
        ...
        $slides = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            SELECT hs.`id_ishitopbanners_slides` as id_slide, hss.`position`, hss.`active`, hss.`textposition`, hss.`textalignment`, hss.`titlecolor`, hss.`subtitlecolor`, hss.`descriptioncolor`, hss.`textbgcolor`,
            hssl.`url`, hssl.`title`, hssl.`subtitle`, hssl.`description`, hssl.`btntitle` ,hssl.`image`
            FROM '._DB_PREFIX_.'ishitopbanners hs
            LEFT JOIN '._DB_PREFIX_.'ishitopbanners_slides hss ON (hs.id_ishitopbanners_slides = hss.id_ishitopbanners_slides)
            LEFT JOIN '._DB_PREFIX_.'ishitopbanners_slides_lang hssl ON (hss.id_ishitopbanners_slides = hssl.id_ishitopbanners_slides)
        ...
        foreach ($slides as &$slide) {
            $slide['image_url'] = $this->context->link->getMediaLink(_MODULE_DIR_.'ishitopbanners/views/img/'.$slide['image']);
        }
    ...
    public function getAllImagesBySlidesId($id_slides, $active = null, $id_shop = null)
    {
        ...
        $results = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            ...
            LEFT JOIN '._DB_PREFIX_.'ishitopbanners_slides hss ON (hs.id_ishitopbanners_slides = hss.id_ishitopbanners_slides)
            LEFT JOIN '._DB_PREFIX_.'ishitopbanners_slides_lang hssl ON (hss.id_ishitopbanners_slides = hssl.id_ishitopbanners_slides)
            WHERE hs.`id_ishitopbanners_slides` = '.(int)$id_slides.' AND hs.`id_shop` = '.(int)$id_shop.
            ($active ? ' AND hss.`active` = 1' : ' ')
        );
    ...
    {
        $req = 'SELECT hs.`id_ishitopbanners_slides` as id_slide
                FROM `'._DB_PREFIX_.'ishitopbanners` hs
                WHERE hs.`id_ishitopbanners_slides` = '.(int)$id_slide;
    ...
    public function renderList()
    {
        ...
        foreach ($slides as $key => $slide) {
            ...
            $associated_shop_ids = IshiTopBanner::getAssociatedIdsShop((int)$slide['id_slide']);
    ...
    public function getAddFieldsValues()
    {
        ...
        $fields['textbgcolor'] = Tools::getValue('textbgcolor', $slide->textbgcolor);
